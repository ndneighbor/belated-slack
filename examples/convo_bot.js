...
controller.hears(['.belate'],['ambient,direct_message,direct_mention,mention'],function(bot,message) {
  bot.startConversation(message, askPeriod);
});
...
askGenre = function(response, convo) {
  convo.ask("What summary type you want?", function(response, convo) {
    convo.say("Ok.")
...
confirmDeliver = function(response, convo) { 
  convo.ask("So you want to get a " + askGenre(response) + " summary from " + askPeriod(response) + " hours ago.", function(response, convo) {
    convo.say("Wonderful, we will get that right to you!");
    convo.next();
  });
}